...
void reduce_kernel(T* output,const T* a, int N, OP op)
{
    ...
    while (n>2)
    {
        ...
        if (threadIdx.x*2<n)
        {
            ...
            {
                float b=shared[threadIdx.x*2+1];
                a=op(a,b);
            }
        }
...
template<Layout dst_layout, Layout src_layout>
__global__
void transpose_kernel(MatrixView<dst_layout> dst,const MatrixView<src_layout> src)
{
    auto src_row=blockIdx.y*blockDim.y+threadIdx.y;
    auto src_col=blockIdx.x*blockDim.x+threadIdx.x;

    if (src.valid_index({src_row,src_col}))
        dst[{src_col,src_row}]=src[{src_row,src_col}];
}
