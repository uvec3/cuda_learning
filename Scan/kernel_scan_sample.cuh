...
__global__ void preScanKernel(int *out, int *in, unsigned size, int *sum)
{
	__shared__ int in_s[BLOCK_SIZE*2];
	int i, idx, offset;
	...
	for(i=1, offset=BLOCK_SIZE ; i<=BLOCK_SIZE; i<<=1, offset>>=1) {
		...
		if(threadIdx.x < i) {
			int t = in_s[offset*(2*threadIdx.x+1)-1];
			in_s[offset*(2*threadIdx.x+1)-1] =
				in_s[offset*(2*threadIdx.x+2)-1];
...
__global__ void addKernel(int *out, int *sum, unsigned size)
{
...
*******************************************************************************/
void preScan(int *out, int *in, unsigned in_size)
{
    // INSERT CODE HERE
	int *sum;
	unsigned num_blocks;
	...
	if(num_blocks > 1) {
		cuda_ret = cudaMalloc((void**)&sum, num_blocks*sizeof(int));
